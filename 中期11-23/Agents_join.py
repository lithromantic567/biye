...
class ELG_A(nn.Module):
    ...
    def forward(self, cur_room_emb, max_length, choose_token_method="sample"):
        ...
        for i in range(max_length):
            ...
            if choose_token_method == "greedy":
                token_idx = torch.argmax(next_token_pred, dim=1) 
                # 计算贪婪选择的 token 的 log 概率
                log_prob = torch.log_softmax(next_token_pred, dim=1)  # 将概率转化为 log 概率
                chosen_log_prob = log_prob[torch.arange(log_prob.size(0)), token_idx]  # 提取选择的 token 的 log 概率
                spoken_token_prob.append(chosen_log_prob)  # 记录 log 概率
    ...
    def cal_loss(self, spoken_token_prob, reward):
        ...
        return loss.mean()
class ELU_B_type(nn.Module):
    def __init__(self):
        super(ELU_B_type, self).__init__()
        ...
        self.fc_type=nn.Sequential(
            nn.Linear(Param.room_emb_size,3)
            )
        self.fc_color=nn.Sequential(
            nn.Linear(Param.room_emb_size,6)
            )
    ...
    def forward(self,  message,choose_room_method="sample",  obj_nums=None):
        #shape从(a,b)到(a,1,b)
    ...
    def cal_loss(self, type_prob, reward):
        ...
        return loss
class ELU_B_color(nn.Module):
    def __init__(self):
        super(ELU_B_color, self).__init__()
        self.voc_embedding = nn.Embedding(Param.voc_size + 2, Param.room_emb_size)
        self.sent_encoder = TransformerEncoder(
            TransformerEncoderLayer(Param.room_emb_size, Param.nhead,dim_feedforward=32),
            num_layers=3
...
class AgentA(nn.Module):
    def __init__(self, type_lang_generate=None, color_lang_generate=None):
        super(AgentA, self).__init__()
        self.type_lang_generate = ELG_A() if type_lang_generate is None else type_lang_generate
        self.color_lang_generate = ELG_A() if color_lang_generate is None else color_lang_generate

    def describe_type(self, cur_room_emb, max_length, choose_token_method="sample"):
        return self.type_lang_generate(cur_room_emb, max_length, choose_token_method)

    def describe_color(self, cur_room_emb, max_length, choose_token_method="sample"):
        return self.color_lang_generate(cur_room_emb, max_length, choose_token_method)

    def cal_type_loss(self, spoken_token_prob, reward):
        return self.type_lang_generate.cal_loss(spoken_token_prob, reward)

    def cal_color_loss(self, spoken_token_prob, reward):
        return self.color_lang_generate.cal_loss(spoken_token_prob, reward)

class AgentB(nn.Module):
    def __init__(self, type_lang_understand=None, color_lang_understand=None):
        super(AgentB, self).__init__()
        self.type_lang_understand = ELU_B_type() if type_lang_understand is None else type_lang_understand
        self.color_lang_understand = ELU_B_color() if color_lang_understand is None else color_lang_understand

    def guess_type(self, message, choose_room_method="sample"):
        return self.type_lang_understand(message, choose_room_method)

    def guess_color(self, message, choose_room_method="sample"):
        return self.color_lang_understand(message, choose_room_method)

    def cal_type_loss(self, type_prob, reward):
        return self.type_lang_understand.cal_loss(type_prob, reward)

    def cal_color_loss(self, color_prob, reward):
        return self.color_lang_understand.cal_loss(color_prob, reward)
