...
from MTAgent_mmoe import MulTaskModel
import torch
import pickle
import numpy as np
...
def guess_room_train(train_dataset,eval_dataset):
    clear_file("Mul_mmoe/Task1")
    clear_file("Mul_mmoe/Task2")
    ...
    for i in range(1000):
        ...
        for input in train_dataloader: 
            ...
            pred_type1.append(type_idxes[0].cpu().numpy())
            # --- BACKWARD ---
            reward_type1 = np.ones_like(type_idxes[0].cpu().numpy())
            reward_type1[type_idxes[0].cpu().numpy() != label1.cpu().numpy() ] = -1
            pred_type2.append(type_idxes[1].cpu().numpy())
            # --- BACKWARD ---
            reward_type2 = np.ones_like(type_idxes[1].cpu().numpy())
            reward_type2[type_idxes[1].cpu().numpy() != label2.cpu().numpy() ] = -1
            #reward *= Param.reward
        ...
        process_result(i,tgt1,pred_type1,total_reward1,total_loss_A1,total_loss_B1,"Mul_mmoe/Task1")
        process_result(i,tgt2,pred_type2,total_reward2,total_loss_A2,total_loss_B2,"Mul_mmoe/Task2")
        
        #task.eval()
        #task.agentA.eval()
        #task.agentB.eval()
        acc_eval1,acc_eval2=guess_room_evaluate(eval_dataset, task,save_path="Mul_mmoe/")
        
        if acc_eval1>best_eval1:
            best_eval1=acc_eval1
            torch.save(task.state_dict(), "Mul_mmoe/Task1/model.pth")
        if acc_eval2>best_eval2:
            best_eval2=acc_eval2
            torch.save(task.state_dict(), "Mul_mmoe/Task2/model.pth")
    ...
    with torch.no_grad():
        ...
        for data,label1,label2 in eval_dataloader:
            ...
            for i in range(5,10):
                ...
                print(type_idxes[0][i],type_idxes[1][i])
